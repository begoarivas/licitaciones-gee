...
modo = st.radio(
    ...
    ["📊 Dashboard histórico", "🎯 Motor de licitaciones"],
    horizontal=True,
)
...
# ============================================================
    ...
    if archivo is not None:
        ...
        with col1:
            st.metric("Oportunidades", f"{len(df_resultado):,}")
        ...
        with col4:
            st.metric(
                "Score ≥ 80",
                int((df_resultado["score_final"] >= 80).sum()),
            )
        ...
        columnas_ranking = [
            col for col in columnas_ranking if col in df_resultado.columns
        ]
        ...
        if not realizar_manual:
            st.info(
                "La evaluación manual es opcional. El ranking automático mostrado arriba es el resultado principal."
            )
            ...
            for _, fila in df_selector.iterrows():
                ...
                expediente = str(fila["Número de expediente"])
                cliente = str(fila["Órgano de Contratación"])
                score = float(fila["score_final"])

                etiqueta = f"{score:.2f} | {expediente} | {cliente}"
            ...
            expediente_seleccionado = expediente_por_etiqueta[seleccion]

            fila_seleccionada = df_resultado[
                df_resultado["Número de expediente"]
                .astype(str)
                .str.strip()
                .eq(expediente_seleccionado.strip())
            ]

            if len(fila_seleccionada) != 1:
                st.error(
                    "No se ha podido identificar de forma única la oportunidad."
                )
                st.stop()
            ...
            st.write(f"**Número de expediente:** {fila['Número de expediente']}")
            st.write(f"**Cliente:** {fila['Órgano de Contratación']}")
            st.write(f"**Objeto:** {fila['Objeto']}")
            st.write(f"**Score automático:** {fila['score_final']:.2f}")
            st.write(
                f"**Presupuesto mensual:** {fila['presupuesto_mensual']:.2f} €/mes"
            )
            ...
            if calcular:
                ...
                try:
                    ...
                    st.error("No se ha podido calcular el score manual.")
                    st.exception(e)
                    st.stop()

                st.divider()

                st.subheader("Resultado de la evaluación manual")
                ...
                datos_economicos = pd.DataFrame(
                    {
                        ...
                        "Importe": [
                            resultado_manual["presupuesto_mensual"],
                            ...
                            resultado_manual["presupuesto_mensual_ajustado"],
                            resultado_manual["ajuste_economico_automatico"],
                ...
                st.write(
                    f"Bonus estratégico: +{resultado_manual['bonus_estrategico']}"
                )

                if resultado_manual["variacion"] < 0:
                    st.warning(
                        f"El score manual baja {abs(resultado_manual['variacion']):.2f} puntos respecto al automático."
                    )

                elif resultado_manual["variacion"] > 0:
                    st.success(
                        f"El score manual aumenta {resultado_manual['variacion']:.2f} puntos respecto al automático."
                    )

                else:
                    st.info(
                        "La evaluación manual no modifica el score automático."
                    )
